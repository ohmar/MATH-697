...
/**
 * Implementing Strassen's algorithm with linked lists
 *
 */
public class LinkedMatrix {
	...
	/**
	 * Computes the size of a matrix and sets width and height accordingly.
	 */
	...
	/**
	 * Split matrix in half vertically. Removes link from left half and right half.
	 * Right half is returned and left half remains.
	 * @return
	 */
	...
	/**
	 * Split matrix in half horizontally. Removes link on upper half from lower half.
	 * Lower half is returned is returned as a new matrix.
	 *
	 * @return
	 */
	...
	/**
	 * Method to mutiply two linked list matrices.
	 *
	 * @param x
	 * @param y
	 * @return
	...
	public static LinkedMatrix multiply(LinkedMatrix x, LinkedMatrix y) {
		if (x.width == 2) {
			int a, b, c, d, e, f, g, h;
			a = x.root.value();            // [0][0]
			b = x.root.east.value();       // [0][1]
			c = x.root.south.value();      // [1][0]
			d = x.root.south.east.value(); // [1][1]

			e = y.root.value();            // [0][0]
			f = y.root.east.value();       // [0][1]
			g = y.root.south.value();      // [1][0]
			h = y.root.south.east.value(); // [1][1]
